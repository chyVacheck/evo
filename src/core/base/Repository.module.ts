 ...
 *
 * @example
 * class SomeCustomRepository extends RepositoryModule {
 *   constructor() {
 *     super(EModuleType.REPOSITORY, 'SomeRepository');
 *   }
 * }
 */
 ...
 * @typeParam TModel - Тип доменной сущности.
 * @typeParam TFilter - Тип фильтра (конкретная БД сузит).
 * @typeParam TUpdate - Тип апдейта (по умолчанию Partial<TModel>).
 */
export abstract class RepositoryModule<
	...
	implements IRepositoryModule<ModuleName, TModel, TFilter, TUpdate>
{
	/**
	 ...
	 */
	abstract aggregate<TRow = unknown>(
		pipeline: unknown,
		opts?: TRepoCommonOpts
	): Promise<TRow[]>;
